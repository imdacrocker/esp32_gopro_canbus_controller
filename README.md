...
5. In both cases, `control_send_set_date_time()` fetches the current UTC from `can_manager_get_utc_ms()`, applies the stored timezone offset via `can_manager_get_tz_offset_hours()` (so cameras receive local time rather than raw UTC), converts the adjusted epoch to calendar fields, and writes a SetDateTime TLV command (ID `0x0D`) to GP-0072 (`cmd_write`). The equivalent adjustment is made in `legacy_control_send_date_time()` for Hero4 cameras before building the `gpControl` date_time HTTP request.
...
| `can_manager` | ESP-IDF v6.0 TWAI driver wrapper. Receives `0x600` (isLogging) and `0x602` (UTC timestamp) frames; broadcasts `0x601` camera status at 5 Hz. Exposes `can_manager_get_utc_ms()` for on-demand UTC retrieval with monotonic-clock extrapolation. Also owns the persistent timezone UTC offset setting: `can_manager_set_tz_offset(hours)` / `can_manager_get_tz_offset_hours()` — stored in NVS (namespace `settings`, key `tz_offset`), loaded at init, default UTC−8. Thread-safe. |
| `wifi_manager` | Soft-AP + HTTP server. Serves the embedded web UI and all `/api/*` endpoints, including the `/api/legacy/*` sub-tree for managing Hero4 cameras and the `GET /api/settings/timezone` / `POST /api/settings/timezone` endpoints for reading and updating the UTC offset. Owns the connected-station table and notifies `legacy_gopro` of L2 association and DHCP events. |
